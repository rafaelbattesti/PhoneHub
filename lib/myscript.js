...
function drawReport(products, cart) {
    var labels = [];
    var cartQty = [];
    var cartRatings = [];
    var cartQoh = []
    ...
    for (var key in cart) {
        for (var i = 0; i < products.length; i++) {
            if (products[i].id == key) {
                ...
                cartQty.push(cart[key]);
                cartRatings.push(products[i].rating);
                cartQoh.push(products[i].qoh);
                break;
            }
        }
    }

    var context = $("#admincanvas").get(0).getContext("2d");
    var data = {
        labels : labels,
        datasets : [
            {
    ...
    var chart = new Chart( context,{ type:'bar', data:data});
}
...
$(document).on("pagebeforeshow", "#admin", function () {
    ...
    drawReport(getLocalStorage("products"), getLocalStorage("cart"));
});
